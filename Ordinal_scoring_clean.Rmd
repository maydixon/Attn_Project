...
write to github
```{r}
write.csv(t.d., file = "Hab_and_eco_sal.csv")
```
...
confint_comp_ls_om<- ggplot(svlst.om, aes(primary_stim,ymin=asymp.LCL,ymax=asymp.UCL,colour= "black"))+
    geom_linerange() +
...
scale_x_discrete(labels=c("0c P. pustulosus","2c P. pustulosus","D. ebraccatus", "R. alata", "rev R. alata", "rev P. pustulosus"))  +
              scale_colour_manual(breaks = c("0c P. pustulosus","2c P. pustulosus","D. ebraccatus", "R. alata", "rev R. alata", "rev P. pustulosus"),
                                values=c(Ts, T1c, T2c, De, Ra, Rra, Rt)) +
...
ylab("Slope of change in responses") +
      ...
      geom_point( data = svlst.om,  mapping =aes(x = svlst.om$primary_stim, y = svlst.om$set_num.trend ), color = "black")
...
#ggsave("habituation_slope_plot_black_ordinal.pdf",width=3, height =4)
```
...
# this computes the paired wilcox0n sign rank for paired two sample data for 5-6 and then 5-7. the [[3]] pulls the p value out of the list produced by wilcox test
...
ra_wilx<- wilcox.test(cast_t.d.$five[cast_t.d.$trial_name_2=="ra_t_ra"], cast_t.d.$six[cast_t.d.$trial_name_2=="ra_t_ra"], paired = TRUE)
ra_wilx
p.adjust(ra_wilx$p.value, method = "holm", 2)

rra_wilx<- wilcox.test(cast_t.d.$five[cast_t.d.$trial_name_2=="rra_rt_rra"], cast_t.d.$six[cast_t.d.$trial_name_2=="rra_rt_rra"], paired = TRUE)
rra_wilx
p.adjust(rra_wilx$p.value, method = "holm", 2)

rra_wilx<- wilcox.test(cast_t.d.$five[cast_t.d.$trial_name_2=="rra_rt_rra"], cast_t.d.$six[cast_t.d.$trial_name_2=="rra_rt_rra"], paired = TRUE)
rra_wilx
p.adjust(0.01463, method = "holm", 2)

tc_wilx<- wilcox.test(cast_t.d.$five[cast_t.d.$trial_name_2=="tc_ts_tc"], cast_t.d.$six[cast_t.d.$trial_name_2=="tc_ts_tc"], paired = TRUE)
tc_wilx
p.adjust(tc_wilx$p.value, method = "holm", 2)

ts_wilx<- wilcox.test(cast_t.d.$five[cast_t.d.$trial_name_2=="ts_tc_ts"], cast_t.d.$six[cast_t.d.$trial_name_2=="ts_tc_ts"], paired = TRUE)
ts_wilx
p.adjust(ts_wilx$p.value, method = "holm", 2)
