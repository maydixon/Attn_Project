...
#Checking out a palette of colors:
        #plotCol(gray(1.1:11 /8))
   gray.colors(8, start = 0.0, end = 0.9, gamma = 2.2, alpha = NULL)  
  grays<- barplot(1:8, col = gray.colors(8, start = 0.0, end = .9, gamma = 1.5, alpha = NULL))  
  grays

#finding the name of those colors:
     barplot(1:7, col =gray(1.5:8/8))
     gray(1.5:8/8)
...
attn_R <- subset(attn_R , attn_R $trial_name_2 != "tia_tib_tia" ) #removes treatment
```
...
write to github
```{r}
write.csv(t.d., file = "Hab_and_eco_sal_nINDV.csv")
```
...
```{r}
#Make a row for sex with Vila and Drake 1 and males 0
t.d.$Sex<-rep(0,nrow(t.d.))

for (i in 1:nrow(t.d.)){
      
  if (t.d.$bat_name[i] == "Sir_Frances_Drake") {
   t.d.$Sex[i]<-"1"
} else if (t.d.$bat_name[i] == "Vila") {
   t.d.$Sex[i]<-"1"
} else 
      t.d.$Sex[i]<-"0"
...
```{r}
#Make a row for sex with Vila and Drake 1 and males 0
t.d.int$Sex<-rep(0,nrow(t.d.int))

for (i in 1:nrow(t.d.int)){
      
  if (t.d.int$bat_name[i] == "Sir_Frances_Drake") {
   t.d.int$Sex[i]<-"1"
} else if (t.d.int$bat_name[i] == "Vila") {
   t.d.int$Sex[i]<-"1"
} else 
      t.d.int$Sex[i]<-"0"
...
```{r}
t.d.int.summary.sex  <- t.d.int %>% group_by (primary_stim, Sex) %>% summarise(mean=mean(as.numeric(Ordinal3)), sd=sd(as.numeric(Ordinal3))) %>% write_csv("initial_interest_by_sex.csv")

t.d.int.summary.sex.2<- t.d.int.summary.sex  %>% group_by(Sex) %>% summarise("mean initial response" = mean(mean))
t.d.int.summary.sex.2 %>% write_csv("initial_interest_by_sex_summary.csv")
...
##not significantly different at 0.05 level. So no random effect needed, keep simpler mode, int_rand
...
cum_lsmeans_primary<-lsmeans::lsmeans(initial_int_primary, "primary_stim", mode = "mean.class")
cum_lsmeans_primary
sv_cum_lsmeans_primary<- cld(cum_lsmeans_primary)
sv_cum_lsmeans_primary
sv_cum_lsmeans_primary %>% as.data.frame() %>% write_csv("initial_interest_estimates.csv")

pairs_lsmeans_primary <-pairs(cum_lsmeans_primary)
pairs_lsmeans_primary 
pairs_lsmeans_primary %>% as.data.frame() %>% write_csv("pairwise_initial_interest.csv")

plot(cum_lsmeans_primary)
# adjusted for a fmaily of 6 estimates. 
...
#reordering for plotting:
      t.d.int$primary_stim <- factor(t.d.int$primary_stim,levels = c("ts" , "t2c" , "de", "ra" , "rra","rt" ),ordered = TRUE)
...
int.plot<- int.plot+ scale_x_discrete(labels=c("0c P. pustulosus","2c pustulosus","D. ebraccatus", "R. alata", "rev R. alata", "rev P. pustulosus"))  +
              scale_fill_manual(breaks = c("0c P. pustulosus", "2c pustulosus","D. ebraccatus", "R. alata", "rev R. alata", "rev P. pustulosus"),
                                values=c(Ts, T2c, De, Ra, Rra, Rt))
...
int.plot_Ordinal_sex<- ggplot(data = t.d.int, aes(x = primary_stim, y = Ordinal3, fill=primary_stim))
int.plot_Ordinal_sex<- int.plot_Ordinal_sex+ geom_boxplot()
int.plot_Ordinal_sex<- int.plot_Ordinal_sex+ geom_point(position = position_jitter(width = 0.1), aes(colour = factor(Sex)))
int.plot_Ordinal_sex<- int.plot_Ordinal_sex+ theme(axis.text.x = element_text(angle = 45, hjust=1, size = 14), legend.position = "none") #format x-axis, and remove side legend #hjust 1 aligns labels
int.plot_Ordinal_sex<- int.plot_Ordinal_sex+ scale_x_discrete(labels=c("0c P. pustulosus","2c pustulosus","D. ebraccatus", "R. alata", "rev R. alata", "rev P. pustulosus"))  +
              scale_fill_manual(breaks = c("0c P. pustulosus","2c pustulosus","D. ebraccatus", "R. alata", "rev R. alata", "rev P. pustulosus"),
                                values=c(Ts, T2c, De, Ra, Rra, Rt))
int.plot_Ordinal_sex<- int.plot_Ordinal_sex + coord_cartesian(ylim = c(0, 5))
int.plot_Ordinal_sex<- int.plot_Ordinal_sex+ theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
panel.background = element_blank(), axis.line = element_line(colour = "black"), axis.title= element_text(size=14))  #gets rid of background
int.plot_Ordinal_sex<- int.plot_Ordinal_sex+ xlab("Acoustic stimuli")
int.plot_Ordinal_sex<- int.plot_Ordinal_sex+ ylab("Ordinal responses in first trial")
int.plot_Ordinal_sex


#ggsave("initial_interest_Ordinal_sex.jpg", width = 6.5, height = 5.55)
```
...
int.plot_Ordinal<- int.plot_Ordinal+ geom_point(position = position_jitter(width = 0.1), aes(colour = factor(Sex)))
...
int.plot_Ordinal<- int.plot_Ordinal+ scale_x_discrete(labels=c("0c P. pustulosus","2c pustulosus","D. ebraccatus", "R. alata", "rev R. alata", "rev P. pustulosus"))  +
              scale_fill_manual(breaks = c("0c P. pustulosus","2c pustulosus","D. ebraccatus", "R. alata", "rev R. alata", "rev P. pustulosus"),
                                values=c(Ts, T2c, De, Ra, Rra, Rt))
...
Int_palette<- c("#006837", "#31a354", "#78c679","#c2e699cc", "#ffffcc")
#scale color brewer, name colors, ylgn 5

t.d.int.plot<- t.d.int

t.d.int.plot$primary_stim <- factor(t.d.int.plot$primary_stim,levels = c("ts" , "t2c" , "de", "ra" , "rra","rt" ),ordered = TRUE)
...
class(t.d.int.plot$primary_stim)   ##R
  ...
  # scale_x_continuous(breaks=c("ts", "t2c", "de",  "ra",  "rra", "rt" ),
     #   labels=c("0c P. pustulosus","2c pustulosus","D. ebraccatus", "R. alata", "rev R. alata", "rev P. pustulosus"))+
...
int.plot_Ordinal2<- int.plot_Ordinal2+ scale_x_discrete(labels=c("0c P. pustulosus","2c pustulosus","D. ebraccatus", "R. alata", "rev R. alata", "rev P. pustulosus"))  +
              scale_fill_manual(breaks = c("0c P. pustulosus","2c pustulosus","D. ebraccatus", "R. alata", "rev R. alata", "rev P. pustulosus"),
                                values=c(Ts, T2c, De, Ra, Rra, Rt))
...
# only trial order as random effect
habit.ordinal3.mod2.5 <- clmm(Ordinal3 ~ set_num:primary_stim + (1| trial_number), data= t.d.hab., Hess=TRUE)
...
anovarandom<- anova(habit.ordinal3.mod.nr, habit.ordinal3.mod2) #comparing no to one effect
anovarandom
anova_effects<-anova(habit.ordinal3.mod2, habit.ordinal3.mod2.5)
anova_effects
anova_trialorder <-anova(habit.ordinal3.mod.nr, habit.ordinal3.mod2.5)
anova_trialorder      
anovarandom1 <- anova(habit.ordinal3.mod2, habit.ordinal3.mod3) #comparing just bat id to both
anovarandom1
...
lst.ord3.intonly=lstrends(habit.ordinal3.intonly, ~primary_stim, var="set_num", transform = "response")
#must not be response transformation, bc works the sme w and without tranform = "response"
lst.ord3.intonly
# Show which are different from one another in letter form:
svlst.ord3.intonly<-cld(lst.ord3.intonly)
svlst.ord3.intonly %>% write.csv("habituation_lsmeans_groups_noindv.csv")


#show pairwise contrasts:
p.lst.ord3.intonly<- pairs(lst.ord3.intonly)
p.lst.ord3.intonly %>% write.csv("habituation_lsmeans_pairwise_noindv.csv")
...
svlst.om$primary_stim<-factor(svlst.om$primary_stim, levels = c("ts" , "t2c" , "de", "ra" , "rra","rt"), ordered=TRUE)
...
scale_x_discrete(labels=c("0c P. pustulosus","2c P. pustulosus","D. ebraccatus", "R. alata", "rev R. alata", "rev P. pustulosus"))  +
              scale_colour_manual(breaks = c("0c P. pustulosus","2c pustulosus","D. ebraccatus", "R. alata", "rev R. alata", "rev P. pustulosus"),
                                values=c(Ts, T1c, T2c, De, Ra, Rra, Rt)) +
#scale_fill_manual(breaks = c("ts","t2c","de", "ra", "rra", "rts"),
                             #   values=c(Ts, T2c, de, ra, rra, rt)) +

 xlab("Call Type") +
ylab("Slope of change in responses") +
      geom_hline(yintercept=0, linetype="dotted") 
      

confint_comp_ls_om
#ggsave("habituation_slope_plot_grayscale_ordinal.eps",width=3, height =4)
```
...
```{r, include = FALSE}
#make O3dinal3 numeric for now
class(t.d.$Ordinal3)
t.d.$Ordinal3 <- as.numeric(t.d.$Ordinal3)
...
#write.csv(subset_O_tia, file = "subset_O_tia.csv")
subset_O_ts <- subset(t.d., t.d.$trial_name_2=="ts_tc_ts" & (t.d.$set_num=="5" |t.d.$set_num=="6" |t.d.$set_num=="7"))


##workaround for casting tia, bc doubled
subset_O_tia_1 <- read_csv("~/GitHub/Attn_Project/Attn_project_github/tia_friedman_1.csv")
subset_O_tia_2 <- read_csv("~/GitHub/Attn_Project/Attn_project_github/tia_friedman_2.csv")
```
...
#subset of all t.d. with only 5-7
subset_t.d. <- subset(t.d.,  (t.d.$set_num=="5" |t.d.$set_num=="6" |t.d.$set_num=="7"))

#cast so 5-7 are their own columns
#didn't work
# t.d_cast <- dcast(subset_t.d., bat_name + primary_stim  ~set_num,)
# View(t.d_cast)

t_de_cast <- dcast(subset_O_t_de, bat_name + trial_name_2 + primary_stim~set_num  ) #reshapes into wide format
de_cast <- dcast(subset_O_de, bat_name + trial_name_2 + primary_stim~set_num )
ra_cast <- dcast(subset_O_ra, bat_name + trial_name_2 + primary_stim ~ set_num)
rra_cast <- dcast(subset_O_rra, bat_name + trial_name_2 + primary_stim ~ set_num)
t_ra_cast <- dcast(subset_O_t_ra, bat_name + trial_name_2 + primary_stim ~ set_num)
rt_cast <- dcast(subset_O_rt, bat_name + trial_name_2 + primary_stim ~ set_num)
tc_cast <- dcast(subset_O_tc, bat_name + trial_name_2 + primary_stim ~ set_num)
ts_cast <- dcast(subset_O_ts, bat_name + trial_name_2 + primary_stim ~ set_num)
tia_cast_1 <- dcast(subset_O_tia_1, bat_name + trial_name_2 + primary_stim~ set_num)
tia_cast_2 <- dcast(subset_O_tia_2, bat_name + trial_name_2 + primary_stim~ set_num)
tia_cast <- rbind(tia_cast_1, tia_cast_2)
#can't do tia for now

#combine into dataset
cast_t.d.<- rbind(de_cast, t_de_cast, ra_cast, rra_cast, t_ra_cast, rt_cast, tc_cast, ts_cast)
```

```{r}
#post reviews, no longer following this strategy
...
```{r}
## Alternatively, as suggested by Reviewer 1, can not make all the comparisons and then do posthocs, but just do a Wilcox matched pairs signed rank test (2 samples matched) on the comparisons I am interested in, 5-7 and 5-6. 

# take a subset of the data, apply two paired Wilcox tests, apply a P-value correction

#put casts into one dataset, keeping trial_name or primary stim or both

#rename cast_t.d. columns
colnames(cast_t.d.) <-  c("bat_name" ,    "trial_name_2" ,"primary_stim", "five", "six", "seven")


# this computes the paired wilcoxin sign rank for paired two sample data for 5-6 and then 5-7. the [[3]] pulls the p value out of the list produced by wilcox test

t.d.wilx<- cast_t.d. %>% group_by(trial_name_2) %>% summarise (w_56 <- wilcox.test( five, six, paired= TRUE)[[3]] , w_57 <- wilcox.test( five, seven, paired= TRUE)[[3]])

#rename clumns to something palatable
colnames(t.d.wilx) <- c("trial_name_2", "w_56", "w_57")

#compute adjusted p_vals using holm method
t.d.wilx <- t.d.wilx %>% group_by(trial_name_2)  %>% mutate(p_adj_56 <- (p.adjust(c(w_56, w_57), method = "holm"))[1],p_adj_57 <- (p.adjust(c(w_56, w_57), method = "holm"))[2] )

colnames(t.d.wilx) <- c("trial_name_2", "w_56_p", "w_57_p", "w_56_p.holm", "w_57_p.holm")

View(t.d.wilx)

#write to output
# t.d.wilx  %>% write_csv('wilcoxin_SRPT_summary_output.csv')
...
```{r}
        ...
        p_ra <- p_ra + geom_point(position = position_jitter(width = 0.1), aes(colour =factor(Sex)))
...
panel.background = element_blank(), axis.line = element_line(colour = "black"), axis.text.y=element_blank(), axis.text.x = element_text( size = 40), axis.title.x=element_blank(),
        ...
        p_ra
 #ggsave("ra_t_trial_ordinal_sex.pdf", height =5, width = 7)             
```
...
```{r}
        ...
        p_t_ra <- p_t_ra + geom_point(position = position_jitter(width = 0.1), aes(colour= Sex))
...
```{r}
        ...
        p_tc <- p_tc + geom_point(position = position_jitter(width = 0.1), aes(colour =factor(Sex)))
...
panel.background = element_blank(), axis.line = element_line(colour = "black"), axis.text.y=element_blank(), axis.text.x = element_text( size = 40), axis.title.x=element_blank(),
        ...
        p_tc
 #ggsave("tc_trial_ordinal_sex.jpg", height =5, width = 7)             
```
...
```{r}
 t.d.just5$primary_stim <- factor(t.d.just5$primary_stim,levels = c("ts" , "t2c" , "de", "ra" , "rra","rt" ),ordered = TRUE)
...
p5<- p5+ scale_x_discrete(labels=c("0c P. pustulosus","2c pustulosus","D. ebraccatus", "R. alata", "rev R. alata", "rev P. pustulosus"))  +
              scale_fill_manual(breaks = c("0c P. pustulosus","2c pustulosus","D. ebraccatus", "R. alata", "rev R. alata", "rev P. pustulosus"),
                                values=c(Ts,  T2c, De, Ra, Rra, Rt))
...
t.d.just5_a$primary_stim <- factor(t.d.just5$primary_stim,levels = c("ts" , "t2c" , "de", "ra" , "rra","rt" ),ordered = TRUE)
 #I'm reversing the ordering of the Ordinal factor, just for plotting, because I think it looks clearer/ more like habituation if the lowest interest score is on the top. 
