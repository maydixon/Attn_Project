...
```{r}
        ...
        initial_int_1 <- glmer(all_tw_set ~ first_stim + (1|trial_number)+(1|Bat_ID), family= "poisson", data = attn_int_2) #model with nested interaction effect/ full model
##     changed it to a non nested random effect structure

      
       initial_int_2 <- glmer(all_tw_set ~ first_stim + (1|Bat_ID), family= "poisson", data = attn_int_2) #model with only subject as random effect #
        ...
        initial_int_3 <- glmer(all_tw_set ~ first_stim -1 + (1|trial_number)+(1|Bat_ID), family= "poisson", data = attn_int_2)  #no intercept
sii3<-summary(initial_int_3)
bootMer(initial_int_3, FUN=fixef, nsim = 3)
#checking convergence issues
#singularity check
diag.vals <- getME(initial_int_3,"theta")[getME(initial_int_3,"lower") == 0]
any(diag.vals < 1e-6) # FALSE, no singularity

#hessian check
## 3. recompute gradient and Hessian with Richardson extrapolation
      devfun <- update(initial_int_3, devFunOnly=TRUE)
...
bootMer(initial_int_restart, FUN=fixef, nsim=3) ## doesn't help

## 5. try all available optimizers

  source(system.file("utils", "allFit.R", package="lme4"))
  allfit.ii3 <- allFit(initial_int_3)
  ...
  ##bobyqa
  initial_int_3 <- glmer(all_tw_set ~ first_stim -1 + (trial_number|Bat_ID), family= "poisson", data = attn_int_2, control=glmerControl(optimizer = "bobyqa"))  #no intercept
  
 iimbob<- bootMer(initial_int_3, FUN=fixef, nsim=3)
 # This is the thing that converges      
sum_int1<-summary(initial_int_1)
sum_int2<- summary(initial_int_2)
sum_int3<- summary(initial_int_3)
sum_int4<- summary(initial_int_4)
int_coef<-cbind(sum_int1$coefficients, sum_int4$coefficients)
boxplot(log(attn_int_2$all_tw_set)~ attn_int_2$first_stim)

int_lsmeans<- lsmeans(initial_int_3, "first_stim")
sv_int3<- cld(int_lsmeans)
plot(int_lsmeans, ~first_stim, comparisons=TRUE)

## negative binomial model. Is this approp? It fits much better. 
initial_int_nb <- glmer.nb(all_tw_set ~ first_stim -1 + (trial_number|Bat_ID),  data = attn_int_2, control=glmerControl(optimizer = "bobyqa"))  #no intercept
int_sum<- summary(initial_int_nb)
anova(initial_int_nb, initial_int_3) #negative binomial fits betta

int_lsmeans.nb<- lsmeans(initial_int_nb, "first_stim")
sv_int_nb<- cld(int_lsmeans.nb)
sv_int_nb
pairs_int_nb<- pairs(int_lsmeans.nb)

######

plot(int_lsmeans.nb, ~first_stim, comparisons=TRUE)

org.sum <- summary(int_lsmeans.nb, infer = c(TRUE,TRUE),
                       level = .95)
org.sum
tat<- transform(org.sum)
tat

#back-transformed # I think this is much less informative, and am going to ignore it. 
int_lsmeans.nb1<- lsmeans(initial_int_nb, "first_stim", transform = "response")
sv_int_nb1<- cld(int_lsmeans.nb1)
sv_int_nb1
sv_int_nb$exp<-with(sv_int_nb, tapply( sv_int_nb$asymp.LCL,sv_int_nb$asymp.LCL, exp))
                    
plot(int_lsmeans.nb1, ~first_stim, comparisons=TRUE)
...
Anova(initial_int_3) #yeah they're different 




boot_int<-bootMer(initial_int_1, FUN=fixef, nsim=4, seed = 102)
confint_int_perc<-confint(initial_int_1)
confint_wald<-confint(initial_int_1, method="Wald")
confint_int<-confint(initial_int_1, method="boot")

write.csv(confint_int, file="confints_initial_perc.csv")
write.csv(confint_wald, file="confints_initial_wald.csv")
write.csv(confint_int, file="confints_initial_boot.csv")
...
CONVENTIONAL ANOVA: 
      ...
      # Randomized Block Design (Bat is the blocking factor) 
      #logged data +1
fit <- aov(y ~ A + B, data=mydataframe)
anova_ini_int<- aov(log(all_tw_set+1) ~ trial_name_2 + bat_name, data=attn_int_2)
anova_ini_int_summary<- summary.aov(anova_ini_int)
anova_ini_int_summary #summary output

#posthocs
 posthoc_ini_int<- TukeyHSD(anova_ini_int) # where fit comes from aov()
...
p <- p + theme(axis.text.x = element_text(angle = 45, hjust=1), legend.position = "none") #format x-axis, and remove side legend #hjust 1 aligns labels
p <- p + scale_x_discrete(labels=c("0c P. pustulosus","1c P. pustulosus","2c pustulosus","D. ebraccatus", "R. alata", "rev R. alata", "rev P. pustulosus"))  +
...
p_t_de <-ggplot(data=t_de_data, aes(x = set_num, y = all_tw_set))      
        ...
        p_t_de <- p_t_de + scale_x_continuous(breaks = c(1,2,3,4,5,6,7)) +
              scale_fill_manual(breaks = c("Ts", "T1c", "T2c", "De", "Ra", "Rra", "Rt"),
       ...
       theme(plot.title = element_text(face="italic")) #this makes the plot title italic
         p_t_de <- p_t_de + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
panel.background = element_blank(), axis.line = element_line(colour = "black")) #this gets rid of grey background and lines, adds 2 axes
...
#trying this by set #
m6<- glmer(all_tw_set ~ set_num + set_num:trial_name_2 + (trial_number| Bat_ID), data= attn_habit_3, family="poisson")
sm6<- summary(m6)
sm6
#now without an intercept, hypothetically to give true slope values. 
m7<- glmer(all_tw_set ~ set_num + set_num:trial_name_2 -1 + (trial_number| Bat_ID), data= attn_habit_3, family="poisson")
sm7<- summary(m7)
sm7
confint_m6<- confint(m6, method="boot")
fixef_m6<- 
...
The *intercept* is the value of the dependent variable when all the independent variables are 0. So in our case, the intercept is the number of twitches at call 0, for whatever the reference is, when the reference cofficient is held at 0 (I think). Pretty similar for all the treatments. 

The *slope/ estimate/ coeffient* value for call number is the main effect of change in the number of twitches with time, with all the differences compared against the reference value (in this case, De). 

To find the slopes of each of the other treatments and how they differ from the slope of De with call number, add the interaction effect to the effect for call number to get the true value. Relevel to get the one that is the current reference. 
...
slope_est <- slope_coeff + (sm2$coefficients[2,1]) #add the interactions to call# coeff (main effect) to get the true values of the slopes
...
### So, now I would like to bootstrap the values for the slopes of the different treatments.
I would like to repeat the models over and over again, x times, so I can get a 95% confidence interval of the true value of the slope. I would then like to plot each of the slopes on a graph, with treatment on the x axis and slope on the y axis, with 95% confidence interval bars around the points. If the 95% intervals do not cross 0, then the slopes are significantly negative. 
```{r}
#mock up of the code: 
#run glmer()
#extract main effect of call number, add the interaction effect to get the slopes for each treatment
  #maybe do this all those times with De as reference, then change the reference value, and do it again with Ra as reference, and just extract the De values (which won't show in the first output bc De values are held constant)
#store the slope values for each treatment somewhere
#run again, x times (1000?)
   # alternatively, I could run and store the main effect and the interaction effects over and over, and then substract each of them after
#calculate 95% confidence interval for the slope of each treatment
#apparently bca is the best way to bootstrap

#alternatively, I could just bootstrap each value independently. 

## ok, so I should keep looking through bolker's worked tutorials. Then I should figure out exactly how I can get my slopes bootstrapped. ((I just want to know if they overlap, and if they are different than 0, not sure how to pull the true slopes from the coefficients ))

#* My best guess is to take the bootstrapped value for the effect of call #, and then add/substract the bootMER values for each of the interactions from it to get the high/low conf intervals. 
- 
bb <- bootMer(m2,
              FUN=function(m2)
              predict(m2,re.form=NA,nsim=20, seed = TRUE, newdata=attn_habit_3,type="response"),
              nsim=500, verbose = TRUE)

just looking at regular confints produced ny lme4
fixeffectsm2<- fixef(m2) #extracting fixed effects from model
m2confint<- confint(m2) #generating conf-ints on fixed effects
plot(m2confint)
m2confint <- data.frame(m2confint)
m2confint <- m2confint[-1,]

lcol <- m2confint[[1]] #pulling out the lower column
ucol <- m2confint[[2]] #pulling out the upper column

dt <- data.frame(fit   = c(fixeffectsm2[1],  fixeffectsm2[1] + fixeffectsm2[2:length(fixeffectsm2)]), 
                 lower = c(lcol[1],  lcol[1] + lcol[2:length(lcol)]), 
                 upper = c(ucol[1],  ucol[1] + ucol[2:length(ucol)]) )
dt$Treatment <- c("D. ebraccatus", "R. alata", "Rev. R. alata", "Rev. P. pustulosus 1c", " P. pustulosus 1c", "P. pustulosus 1c", "P. pustulosus 2c", "P. pustulosus 1c", "P. pustulosus 0c") 
LETTERS[1:nrow(dt)]
dt<- dt[, -4]
plot<-ggplot(dt, aes(x = batch, y = fit, ymin = lower, ymax = upper)) +
  
  geom_errorbar(width = 0.2) + geom_point() + theme_bw()

#^^ apparently this method is wrong bc it doesn't account for covariences in the coefficients. The following model apparently  accounts for that by not having an intercept? https://stats.stackexchange.com/questions/117641/how-trustworthy-are-the-confidence-intervals-for-lmer-objects-through-effects-pa 
fm2 <- lmer(strength ~ batch - 1 + (1 | cask), Pastes)

m2_ni<- glmer(all_tw_call ~ call_num + call_num:trial_name_2 -1 + (trial_number| Bat_ID), data= attn_habit_3, family="poisson") #mostly won't converge

m2_ni<- glmer(all_tw_call ~ call_num + call_num:trial_name_2 -1 + (1| Bat_ID), data= attn_habit_3, family="poisson")#trying to run model w/0 intercept, so coefficients are true values, took out trial number as random variable

m2_ni_confint <- confint(m2_ni,method="boot") #calculating confint
s_m2_ni<- summary(m2_ni_confint)
fixef_m2_ni <- fixef(m2_ni)
fixef_m2_ni_1<- fixef_m2_ni[-1]
m2_ni_confintdf <- data.frame(m2_ni_confint)
m2_ni_confintdf1 <- m2_ni_confintdf[-1:-2,]

lcol <- m2_ni_confintdf1[[1]] #pulling out the lower column
ucol <- m2_ni_confintdf1[[2]] #pulling out the upper column

dt <- data.frame(fit   = fixef_m2_ni_1, 
                 lower = lcol, 
                 upper = ucol )
dt$Treatment <- c("D. ebraccatus", "R. alata", "Rev. R. alata", "Rev. P. pustulosus 1c", " P. pustulosus 1c", "P. pustulosus 1c", "P. pustulosus 2c", "P. pustulosus 1c" )


plot<-ggplot(dt, aes(x = fixef_m2_ni_1, y = fit, ymin = lower, ymax = upper)) +
  
  geom_errorbar(width = 0.01) + geom_point() + theme_bw()
plot
#
#This is another package that calculates confidence intervals. run this and see how it compares:
library("lsmeans")
lsmeans_m2_ni<- lsmeans(m2_ni, "trial_name_2")
sum_lsmeans<- summary(lsmeans_m2_ni)
sum_lsmeans

lsmeans_m2_ni_2<- lsmeans(m2_ni, "trial_name_2", transform="response")
sum_lsmeans2<- summary(lsmeans_m2_ni_2)
sum_lsmeans2

# Figure out what transform means
# try with all_tw_set
#0 inflated model?
#make a data thing with regular standard error as well as the boot strapped standard error
...
library(ggplot2)
    # New habituation plots (boxplots)
        ...
        #If you want to look at habituation alone, use the following code. Hash out if you want discrimination as well:
        
       # attn_whole_set <- subset(attn_whole_set, attn_whole_set$set_num != "6" & attn_whole_set$set_num != "7") #removing last 2 sets
        #then do a "find all", and replace all attn_whole_set with attn_whole_set_1to5
...
#Checking out a palette of colors:
        plotCol(gray(.5:7.8 /9)
#finding the name of those colors:
      gray(.5:7.8 /9)
#Assigning colors to the different treatments to use in the following graphs. This way I can easily change the colors. 
...
panel.background = element_blank(), axis.line = element_line(colour = "black")) #this gets rid of grey background and lines, adds 2 axes
        ...
        p_rra <- p_rra + scale_x_continuous(breaks = c(1,2,3,4,5,6,7)) +
              scale_fill_manual(breaks = c("1", "2", "3", "4", "5", "6", "7"),
                                values=c( Rra, Rra, Rra, Rra, Rra, Rt, Rra ))
...
panel.background = element_blank(), axis.line = element_line(colour = "black")) #this gets rid of grey background and lines, adds 2 axes
        ...
        p_tc<- p_tc + labs(title="2-chuck P. pustulosus / 0-chuck P. pustulosus") +
       theme(plot.title = element_text(face="italic")) #this makes the plot title italic
...
panel.background = element_blank(), axis.line = element_line(colour = "black")) #this gets rid of grey background and lines, adds 2 axes
        ...
        p_ts<- p_ts + labs(title="0-chuck P. pustulosus / 2-chuck P. pustulosus")+
       theme(plot.title = element_text(face="italic")) #this makes the plot title italic
...
panel.background = element_blank(), axis.line = element_line(colour = "black")) #this gets rid of grey background and lines, adds 2 axes
        ...
        #### plots without x and y titles for a combined plot
        ...
        p_de<- p_de + labs(title = "D. ebraccatus / 0-chuck P. pustulosus") +
         ...
         p_de <- p_de + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
panel.background = element_blank(), axis.line = element_line(colour = "black")) #this gets rid of grey background and lines, adds 2 axes
         p_de<- p_de +scale_y_continuous(name= "", breaks=c(0,10,20), limits=c(0,25)) +
               theme(axis.text.x = element_text(size=14), axis.text.y = element_text(size=14)) #setting the size of the numbering, the limits of the y axes
        p_de <- p_de + stat_smooth(colour="black", size=0.5, method="lm", se=FALSE ,  data= subset(de_data, de_data$set_num!="6" & de_data$set_num!="7")) 
        p_de
       ...
       theme(plot.title = element_text(face="italic")) #this makes the plot title italic
         p_t_de <- p_t_de + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
panel.background = element_blank(), axis.line = element_line(colour = "black")) #this gets rid of grey background and lines, adds 2 axes
         p_t_de<- p_t_de +scale_y_continuous(name= "", breaks=c(0,10,20), limits=c(0,25)) +
               theme(axis.text.x = element_text(size=14), axis.text.y = element_text(size=14)) #setting the size of the numbering, the limits of the y axes
        p_t_de <- p_t_de + stat_smooth(colour="black", size=0.5, method="lm", se=FALSE ,  data= subset(t_de_data, t_de_data$set_num!="6" & t_de_data$set_num!="7")) 
        p_t_de
       ...
       theme(plot.title = element_text(face="italic")) #this makes the plot title italic
        p_rra <- p_rra + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
panel.background = element_blank(), axis.line = element_line(colour = "black")) #this gets rid of grey background and lines, adds 2 axes
        p_rra<- p_rra  +scale_y_continuous(name= "", breaks=c(0,10,20), limits=c(0,25)) +                theme(axis.text.x = element_text(size=14), axis.text.y = element_text(size=14)) #setting the size of the numbering, the limits of the y axes
        p_rra <- p_rra + stat_smooth(colour="black", size=0.5, method="lm", se=FALSE ,  data= subset(rra_data, rra_data$set_num!="6" & rra_data$set_num!="7")) 
        p_rra
       ...
       theme(plot.title = element_text(face="italic")) #this makes the plot title italic
        p_ra <- p_ra + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
panel.background = element_blank(), axis.line = element_line(colour = "black")) #this gets rid of grey background and lines, adds 2 axes
        p_ra<- p_ra +scale_y_continuous(name= "", breaks=c(0,10,20), limits=c(0,25)) +                theme(axis.text.x = element_text(size=14), axis.text.y = element_text(size=14)) #setting the size of the numbering, the limits of the y axes #sets y limit
        p_ra <- p_ra + stat_smooth(colour="black", size=0.5, method="lm", se=FALSE ,  data= subset(ra_data, ra_data$set_num!="6" & ra_data$set_num!="7")) 
       ...
       theme(plot.title = element_text(face="italic")) #this makes the plot title italic
        p_t_ra <- p_t_ra + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
panel.background = element_blank(), axis.line = element_line(colour = "black")) #this gets rid of grey background and lines, adds 2 axes
        p_t_ra<- p_t_ra +scale_y_continuous(name= "", breaks=c(0,10,20), limits=c(0,25)) +                theme(axis.text.x = element_text(size=14), axis.text.y = element_text(size=14)) #setting the size of the numbering, the limits of the y axes #sets y limit
        p_t_ra <- p_t_ra + stat_smooth(colour="black", size=0.5, method="lm", se=FALSE ,  data= subset(t_ra_data, t_ra_data$set_num!="6" & t_ra_data$set_num!="7")) 
        p_t_ra
       ...
       theme(plot.title = element_text(face="italic")) #this makes the plot title italic
        p_rt <- p_rt + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
panel.background = element_blank(), axis.line = element_line(colour = "black")) #this gets rid of grey background and lines, adds 2 axes
        p_rt<- p_rt +scale_y_continuous(name= "", breaks=c(0,10,20), limits=c(0,25)) +                theme(axis.text.x = element_text(size=14), axis.text.y = element_text(size=14)) #setting the size of the numbering, the limits of the y axes #sets y limit
        p_rt <- p_rt + stat_smooth(colour="black", size=0.5, method="lm", se=FALSE ,  data= subset(rt_data, rt_data$set_num!="6" & rt_data$set_num!="7")) 
        p_rt
       ...
       theme(plot.title = element_text(face="italic")) #this makes the plot title italic
        p_tc <- p_tc + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
panel.background = element_blank(), axis.line = element_line(colour = "black")) #this gets rid of grey background and lines, adds 2 axes
        p_tc<- p_tc +scale_y_continuous(name= "", breaks=c(0,10,20), limits=c(0,25)) +                theme(axis.text.x = element_text(size=14), axis.text.y = element_text(size=14)) #setting the size of the numbering, the limits of the y axes #sets y limit
        p_tc <- p_tc + stat_smooth(colour="black", size=0.5, method="lm", se=FALSE ,  data= subset(tc_data, tc_data$set_num!="6" & tc_data$set_num!="7")) 
        p_tc
       ...
       theme(plot.title = element_text(face="italic")) #this makes the plot title italic
        p_ts <- p_ts + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
panel.background = element_blank(), axis.line = element_line(colour = "black")) #this gets rid of grey background and lines, adds 2 axes
        p_ts<- p_ts +scale_y_continuous(name= "", breaks=c(0,10,20), limits=c(0,25)) +                theme(axis.text.x = element_text(size=14), axis.text.y = element_text(size=14)) #setting the size of the numbering, the limits of the y axes #sets y limit
        p_ts <- p_ts + stat_smooth(colour="black", size=0.5, method="lm", se=FALSE ,  data= subset(ts_data, ts_data$set_num!="6" & ts_data$set_num!="7")) 
        p_ts
       ...
       theme(plot.title = element_text(face="italic")) #this makes the plot title italic
        p_tia <- p_tia + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
panel.background = element_blank(), axis.line = element_line(colour = "black")) #this gets rid of grey background and lines, adds 2 axes
        p_tia<- p_tia +scale_y_continuous(name= "", breaks=c(0,10,20), limits=c(0,25)) +                theme(axis.text.x = element_text(size=14), axis.text.y = element_text(size=14)) #setting the size of the numbering, the limits of the y axes #sets y limit
        p_tia <- p_tia + stat_smooth(colour="black", size=0.5, method="lm", se=FALSE ,  data= subset(tia_data, tia_data$set_num!="6" & tia_data$set_num!="7"))
        p_tia
        #430 x 295 export
...
#with logged +1 (for zero) data
p <- ggplot(data = attn_set_5, aes(x = primary_stim, y = log(all_tw_set), fill=primary_stim))
p <- p + geom_boxplot()
p <- p + geom_point(position = position_jitter(width = 0.1))
