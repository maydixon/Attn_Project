...
habit.ordinal.mod <- clmm(Ordinal ~ set_num:primary_stim + (1| Bat_ID), data= t.d., Hess=TRUE, nAGQ=10)
...
#no random effect: 
habit.ordinal.mod.nr<- clm(Ordinal ~ set_num*primary_stim, data= t.d.)

#one random effect
habit.ordinal.mod2 <- clmm(Ordinal ~ set_num*primary_stim + (1| Bat_ID), data= t.d., Hess=TRUE)


#two random effects
habit.ordinal.mod3 <- clmm(Ordinal ~ set_num*primary_stim + (1| Bat_ID) + (1 | trial_number), data= t.d., Hess=TRUE)


#two random effects, nested
habit.ordinal.mod4 <- clmm(Ordinal ~ set_num*primary_stim + (trial_number| Bat_ID), data= t.d., Hess=TRUE)
summary(habit.ordinal.mod4)
#compare models

anovarandom<- anova(habit.ordinal.mod.nr, habit.ordinal.mod2)
anovarandom
anovarandom1 <- anova(habit.ordinal.mod2, habit.ordinal.mod3)
anovarandom1
anovarandom2 <- anova(habit.ordinal.mod3, habit.ordinal.mod4) 
anovarandom2
## all together now
...
habit.ordinal.intonly<- clmm(Ordinal ~ set_num:primary_stim + (trial_number| Bat_ID), data= t.d., Hess=TRUE)

habit.ordinal.modfull <- clmm(Ordinal ~ set_num*primary_stim + (trial_number| Bat_ID), data= t.d., Hess=TRUE)
...
comp_meth <- ggplot(data= t.d., aes(x=Ordinal, y=all_tw_set, color=primary_stim))
...
comp_treat <- ggplot(data= t.d., aes(x = primary_stim, y = Ordinal, color=primary_stim))
...
Rt  ="#C9C9C9"
##
svlst.o<-svlst.ord.hab #ordering factor to change position of plots
svlst.o$primary_stim<-factor(svlst.o$primary_stim, levels = c("ts" ,"t1c", "t2c" , "de", "ra" , "rra","rt"), ordered=TRUE)

  theme_set(theme_classic())    

confint_comp_ls_o<- ggplot(svlst.o, aes(primary_stim,ymin=asymp.LCL,ymax=asymp.UCL,colour=primary_stim))+
    geom_linerange() +
...
```
# double check plots to make sure they are all asically linear and negative (so that time points are weird as linear)
########################
...
#pull partial string match  x.1 for call number using grep, this means only 1 set will be called, avoids redundant data
...
```{r}
        ...
        #Ra_T
        ra_t_data <- attn_whole_set[attn_whole_set$trial_name_2=="ra_t_ra", ]       
        p_ra <- ggplot(data=ra_t_data, aes(x = set_num, y = all_tw_set))      
...
panel.background = element_blank(), axis.line = element_line(colour = "black"), axis.text.y=element_blank(), axis.text.x = element_text( size = 40), axis.title.x=element_blank(),
          axis.title.y=element_blank()) #this gets rid of grey background and lines, adds 2 axes
        p_ra <- p_ra + coord_cartesian(ylim = c(0, 25))
...
panel.background = element_blank(), axis.line = element_line(colour = "black"), axis.text.y=element_blank(), axis.text.x = element_text( size = 40), axis.title.x=element_blank(),
          axis.title.y=element_blank()) #this gets rid of grey background and lines, adds 2 axes
        p_t_ra <- p_t_ra + coord_cartesian(ylim = c(0, 25))
...
panel.background = element_blank(), axis.line = element_line(colour = "black"), axis.text.y=element_blank(), axis.text.x = element_text( size = 40), axis.title.x=element_blank(),
          axis.title.y=element_blank()) #this gets rid of grey background and lines, adds 2 axes
        p_tia <- p_tia + coord_cartesian(ylim = c(0, 25))
...
panel.background = element_blank(), axis.line = element_line(colour = "black"), axis.text.y=element_blank(), axis.text.x = element_text( size = 40), axis.title.x=element_blank(),
          axis.title.y=element_blank()) #this gets rid of grey background and lines, adds 2 axes
        p_ts <- p_ts + coord_cartesian(ylim = c(0, 25))
...
panel.background = element_blank(), axis.line = element_line(colour = "black"), axis.text.y=element_blank(), axis.text.x = element_text( size = 40), axis.title.x=element_blank(),
          axis.title.y=element_blank()) #this gets rid of grey background and lines, adds 2 axes
        p_tc <- p_tc + coord_cartesian(ylim = c(0, 25))
...
```{r}
        ...
        #rt
        rt_data <- attn_whole_set[attn_whole_set$trial_name_2=="rt_rra_rt", ]       
...
panel.background = element_blank(), axis.line = element_line(colour = "black"), axis.text.y=element_blank(), axis.text.x = element_text( size = 40), axis.title.x=element_blank(),
          axis.title.y=element_blank()) #this gets rid of grey background and lines, adds 2 axes
        p_rt <- p_rt + coord_cartesian(ylim = c(0, 25))
...
trial5mod<- glmer(all_tw_set ~ primary_stim -1 + (trial_number|Bat_ID), family= "poisson", data = attn_set_5) 
